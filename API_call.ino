...
void getFinnhubStockData() {
  ...
  String finnhubApiUrl = "https://finnhub.io/api/v1/quote?symbol=AAPL&token=" + String(finnhubApiKey); // Example symbol (AAPL)
  ...
  if (httpCode > 0) {
    if (httpCode == HTTP_CODE_OK) {
      ...
      if (doc.containsKey("c") && doc.containsKey("pc") && doc.containsKey("t")) {
        ...
        time_t convertedTime = timestamp / 1000;
        struct tm *tmInfo = gmtime(&convertedTime);
        Serial.printf(", Converted Time: %04d-%02d-%02d %02d:%02d:%02d", tmInfo->tm_year + 1900, tmInfo->tm_mon + 1, tmInfo->tm_mday, tmInfo->tm_hour, tmInfo->tm_min, tmInfo->tm_sec);
